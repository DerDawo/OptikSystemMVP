...
import { Box, Button, Card, Checkbox, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Divider, FormControlLabel, IconButton, ListItem, ListItemButton, ListItemText, Stack, TextField as MuiTextField, Theme, Typography, useMediaQuery } from '@mui/material';
...
const getMessageFieldKey = (record: Record<string, unknown>, candidateFields: string[]) => {
    const keys = Object.keys(record);

    for (const fieldName of candidateFields) {
        const match = keys.find(key => key.toLowerCase() === fieldName.toLowerCase());
...
const createTemplateDraftFromRecord = (record: NachrichtRecord): MessageTemplateDraft => ({
    id: record.id,
    titleKey: getMessageFieldKey(record, messageTitleFields) ?? defaultTemplateTitleKey,
    contentKey: getMessageFieldKey(record, messageContentFields) ?? defaultTemplateContentKey,
...
export const KundeMessage = () => {
    ...
    const [templateToDelete, setTemplateToDelete] = useState<NachrichtRecord | null>(null);
    ...
    const loadMessages = useCallback(async (preferredKey?: string | null) => {
        ...
        setSelectedMessageKey(previousKey => {
            ...
            return targetKey && nextMessages.some(message => message._rowKey === targetKey)
                ? targetKey
    ...
    const handleSaveTemplate = async () => {
        ...
        const { data, error } = templateDraft.id !== undefined
            ? await supabase.from('nachrichten').update(payload).eq('id', templateDraft.id).select('id').maybeSingle()
            : await supabase.from('nachrichten').insert(payload).select('id').maybeSingle();
        ...
        const savedId = templateDraft.id ?? (data as { id?: string | number } | null)?.id;
    ...
    const handleConfirmDeleteTemplate = async () => {
        ...
        const { error } = await supabase.from('nachrichten').delete().eq('id', templateToDelete.id);
    ...
    return (
        <Box sx={{ width: '100%', maxWidth: 1080, margin: '0 auto', minWidth: 0, padding: { xs: '0.75em', md: '1.5em' } }}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: '1em', minWidth: 0 }}>
                    <Card sx={{ padding: '1em' }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1 }}>
                            ...
                            <Button size='small' startIcon={<AddIcon />} onClick={openCreateTemplateDialog}>
                                Neue Vorlage
                            </Button>
                        </Box>
                        <Divider sx={{ mb: 1 }} />
            ...
            <Dialog open={templateDialogOpen} onClose={closeTemplateDialog} fullWidth maxWidth='sm'>
                <DialogTitle>
                    {templateDraft.id !== undefined ? 'Vorlage bearbeiten' : 'Neue Vorlage anlegen'}
                </DialogTitle>
                <DialogContent>
                    <Stack spacing={2} sx={{ mt: 1 }}>
                        <MuiTextField
                            ...
                            onChange={event => setTemplateDraft(previous => ({ ...previous, titel: event.target.value }))}
                            fullWidth
                            autoFocus
                        />
                        <MuiTextField
                            ...
                            onChange={event => setTemplateDraft(previous => ({ ...previous, text: event.target.value }))}
                            fullWidth
                            multiline
                            minRows={4}
            ...
            <Dialog open={templateToDelete !== null} onClose={closeDeleteTemplateDialog}>
                <DialogTitle>Vorlage löschen</DialogTitle>
                <DialogContent>
                    <DialogContentText>
                        Soll die Vorlage „{templateToDelete ? getMessageTitle(templateToDelete) : ''}“ wirklich gelöscht werden?
                    </DialogContentText>
                    {templateDeleteError ? (
                        <Typography color='error' sx={{ mt: 1 }}>{templateDeleteError}</Typography>
                    ) : null}
                </DialogContent>
                <DialogActions>
                    <Button onClick={closeDeleteTemplateDialog} disabled={isDeletingTemplate}>
                        Abbrechen
                    </Button>
                    <Button
                        color='error'
